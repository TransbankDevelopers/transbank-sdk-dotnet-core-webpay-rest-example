...
namespace Controllers.Patpass
{
    ...
    public class PatpassComercioController : BaseController
    {
        ...
        public ActionResult Start()
        {
            ...
            string returnUrl = "https://mvargas:5001/patpass_comercio/commit";//request.getRequestURL().toString().replace("start","commit"); // urlHelper.Action("commit", "patpass_comercio", null, Request.Scheme);
            string name = "Isaac";
            string lastName = "Newton";
            string secondLastName = "Gonzales";
            string rut = "11111111-1";
            string serviceId = "Service_" + GetRandomNumber();
            string finalUrl = "https://mvargas:5001/patpass_comercio/voucher_return"; //request.getRequestURL().toString().replace("start","voucher_return");//urlHelper.Action("voucher_return", "patpass_comercio", null, Request.Scheme);
